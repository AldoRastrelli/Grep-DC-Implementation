...
/*
gcc -g -std=c99 -Wall -Wtype-limits -pedantic -Wconversion -Wno-sign-conversion *.c -o pruebas
valgrind --leak-check=full --track-origins=yes --show-reachable=yes ./pruebas hola 1 ejemplo.txt
*/
    ...
    while (fgets(linea,TAM_MAX,archivo) != NULL){
        ...
        if (search_rabinkarp(buscado,linea,NUM_PRIMO)>=0){
            while (!lista_esta_vacia(lista)){
                char* borrado = lista_borrar_primero(lista);
                fputs(borrado,stdout);
                free(borrado);
            }
        }
    }
...
int main(int argc, char* argv[]){
    ...
    int lineas_previas;
    if ((lineas_previas = chequeo_contexto(argv)) < 0){
